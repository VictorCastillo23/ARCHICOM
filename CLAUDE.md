...
- **Backend = Supabase, YA desplegado.** 9 tablas con RLS, RPC `SECURITY DEFINER` (`aceptar_solicitud` / `rechazar_solicitud`), vista `feed_publicaciones`, trigger `handle_new_user`, bucket `publicaciones`. **No modifiques el esquema sin autorizacion explisita** (tablas, columnas, policies). Para introspección o el endurecimiento del §7 (BD doc), usa el **MCP `supabase`**, no SQL a ciegas.
  ...
  > **Excepción documentada (feed-card-file-preview)**: esquema extendido de forma ADITIVA (aprobado explícitamente). Columna nullable `publicacion.archivo_thumbnail_url text` — miniatura JPEG (página 1) de un PDF, generada **client-side** (`pdfjs-dist` + `@napi-rs/canvas`, ver `lib/pdf/generateThumbnail.ts`) al publicar/editar y subida junto al archivo principal al mismo bucket `publicaciones`; `NULL` para imágenes (la propia imagen es la miniatura) o PDFs no re-guardados desde este cambio. `publicacion` usa GRANT de tabla completo (no por columna, a diferencia de `usuario` — ver footgun de abajo), sin `GRANT` adicional necesario. Vistas `feed_publicaciones` y `feed_trending` recreadas (`security_invoker=true` preservado) para exponerla. Sin cambios en RLS/RPC/policies; `autor_id` sigue saliendo de sesión. Ver `Vitrina_BD_Conexion_Backend.md` §3.20b.
