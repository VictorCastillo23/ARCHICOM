  ...
  return (
    <div className="animate-page">
      <div className="flex items-center justify-between mb-6 pb-4 border-b border-border">
        <h1 className="text-(length:--size-heading-md) font-display font-normal">Revistas</h1>
      </div>
              ...
              <Link key={rev.id} href={`/admin/revistas/${rev.id}`} className="block group">
                <Card
                  className={`group-hover:shadow-md transition-shadow border-l-4 ${
                    isActiva
                      ? 'border-l-primary'
                      : 'border-l-transparent group-hover:border-l-primary'
                  }`}
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      <h2 className="font-display font-normal text-(length:--size-heading-sm) truncate">
                        {rev.titulo}
                      </h2>
                      {rev.volumen && (
                        <p className="text-sm text-text-muted">Vol. {rev.volumen}</p>
                      )}
                    </div>
