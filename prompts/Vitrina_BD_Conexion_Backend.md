...
La UI del form solo expone destinatarios `todos` / `ids` (no `ciudad` — decisión explícita, ver los docs de arriba); el tipo `DestinatariosCriterio` y la RPC siguen soportando `ciudad` a nivel de contrato para uso directo de la API.
